...
public class APRController : MonoBehaviour
{
    ...
    void PlayerMovement()
    {
        ...
        if (forwardIsCameraDirection && Move)
        {
            Direction = APR_Parts[0].transform.TransformDirection(new Vector3(0f, 0f, speed)); //APR_Parts[0].transform.rotation * new Vector3(0, 0f, 0.3f);
    ...
    void PlayerRotation()
    {
        if(forwardIsCameraDirection)
        {
            ...
            Quaternion.Slerp(APR_Parts[0].GetComponent<ConfigurableJoint>().targetRotation, Quaternion.Euler(new Vector3(0, 360 - MovementScript.CalculatedAngle, 0)), Time.deltaTime * turnSpeed);
        }
    }
